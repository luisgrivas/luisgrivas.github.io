...
A partir de este resultado que la esfera $S^2$ *no se puede peinar* continuamente sin formar algún remolino. 
...
Existe una relación sencilla entre el grupo fundamental $\pi_1(X)$  de un espacio $X$ y su primer grupo de homología $H_1(X)$: el primer grupo de homología de $X$ es la [abelianización](https://en.wikipedia.org/wiki/Commutator_subgroup#Abelianization) del grupo fundamental de $X$. :exclamation:
...
Probaremos el resultado mediante inducción sobre $n$. En la sección anterior se mostró que $H_0(S^1) \cong H_1(S^1) \cong \mathbb{Z}$. Además, como $H_p(S^1) \cong H_{p-1}(S^0)$ para $p > 1$ y $S^0$ consta de dos puntos, $H_{p-1}(S^0) = 0$. 

Suponga que el resultado es cierto para toda esfera $S^{n-1}$. De igual manera, por los resultados de la sección anterior, $H_0(S^n) = \mathbb Z$ y $H_1(S^n) = 0$. Para $p > 1$, la hipótesis de inducción establece que
...
Si $Id_{S^n}: S^n \rightarrow S^n$ es el mapeo identidad, entonces su grado es igual a $1$.   Sea $\alpha: S^n \rightarrow S^n$ el mapeo antipodal $x \mapsto -x$. Su grado es igual a $(-1)^{n+1}$. Por tanto, $\alpha$ es homotópico a la identidad si y solo si $n$ es impar. :exclamation:
...
*Demostración.* Supóngase que existiera tal campo vectorial $V$. Podemos asumir que $\lvert V(x) \rvert = 1$ para toda $x \in S^n$. La función $H: S^n \times I \rightarrow S^n$ definida como 
